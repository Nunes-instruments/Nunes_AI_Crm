...
$config=Join-Path $common 'client.json';@{server_url=$server;user_id=$user.id;user_name=$user.name;device_token=$token;device_type='OWNER';configured_at=(Get-Date).ToString('o');client_version='2.11.4'}|ConvertTo-Json|Set-Content -LiteralPath $config -Encoding UTF8
