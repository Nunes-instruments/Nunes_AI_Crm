...
@{server_url=$server;server_candidates=@($candidates);user_id=$id;user_name=$user.name;device_token=$deviceToken;device_type='STAFF';configured_at=(Get-Date).ToString('o');client_version='2.11.13'} | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $config -Encoding UTF8
